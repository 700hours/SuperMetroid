...
{
	public class CeresEmptyDoor : ModTile
	{
		public override void SetDefaults()
		{
			...
			name.SetDefault("Ceres Empty Door");
		...
		public override void PostDraw(int i, int j, SpriteBatch SB)
		{
			if(!initialize)
			{
				Main.PlaySound(mod.GetLegacySoundSlot(SoundType.Custom, "Sounds/Custom/DoorOpening"), new Vector2(i*16, j*16));
				initialize = true;
			}
			...
			if(GlobalPlayer.tileTime <= 0)
			{
				...
				WorldGen.PlaceTile(i, j, mod.TileType("CeresDoor"));
			}
		}
	}
}