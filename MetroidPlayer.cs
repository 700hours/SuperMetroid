...
namespace SuperMetroid
{
	...
	{
		public bool
			enabledSuit = false, Respawn, Switched = false,  morph, crit, correctTile, isLighted,
			PleaseDraw = true,
	...
	/*	useful for saving constant variable values
		...
		// 	player run boost
			...
			int tileX = (int)TileV.X;
			int tileY = (int)TileV.Y;
			...
			if(runBoost && (onGround || onGroundDeep)) player.maxRunSpeed += 1.3f;
		...
		#region item tiles
			...
			int TileX = (int)tilev.X;
			int TileY = (int)tilev.Y+1;	// correcting offset from Ceres Door positioning
		
		//charge beam	
			int type2 = mod.TileType("ChargeBeam");
			if(CheckTile(TileX, TileY, (ushort)type2))
			{
				haveCharge = true;
				Item.NewItem(new Vector2(TileX*16,TileY*16),8,8,mod.ItemType("ChargeBeam"),1,false,-1,false,false);
				upgradePercent++;
				for(int i = 1; i < 3; i++) 
				{
					if(Main.tile[TileX, TileY+i].type == type2) WorldGen.KillTile(TileX, TileY+i, false, false, true);
				}
				if(ffTimer <= 0)
				{
		...
		//wave beam
			int type3 = mod.TileType("WaveBeam");
			if(CheckTile(TileX, TileY, (ushort)type3))
			{
				haveWave = true;
				Item.NewItem(new Vector2(TileX*16,TileY*16),8,8,mod.ItemType("WaveBeam"),1,false,-1,false,false);
				upgradePercent++;
				for(int i = 1; i < 3; i++) 
				{
					if(Main.tile[TileX, TileY+i].type == type3) WorldGen.KillTile(TileX, TileY+i, false, false, true);
				}
				if(ffTimer <= 0)
				{
					ffTimer = 3600; //1 minute
		...
		//spazer
			int type4 = mod.TileType("Spazer");
			if(CheckTile(TileX, TileY, (ushort)type4))
			{
				haveSpazer = true;
				Item.NewItem(new Vector2(TileX*16,TileY*16),8,8,mod.ItemType("Spazer"),1,false,-1,false,false);
				upgradePercent++;
				for(int i = 1; i < 3; i++) 
				{
					if(Main.tile[TileX, TileY+i].type == type4) WorldGen.KillTile(TileX, TileY+i, false, false, true);
				}
				if(ffTimer <= 0)
				{
					ffTimer = 3600; //1 minute
		...
		//ice beam
			int type5 = mod.TileType("IceBeam");
			if(CheckTile(TileX, TileY, (ushort)type5))
			{
				haveIce = true;
				Item.NewItem(new Vector2(TileX*16,TileY*16),8,8,mod.ItemType("IceBeam"),1,false,-1,false,false);
				upgradePercent++;
				for(int i = 1; i < 3; i++) 
				{
					if(Main.tile[TileX, TileY+i].type == type5) WorldGen.KillTile(TileX, TileY+i, false, false, true);
				}
				if(ffTimer <= 0)
				{
					ffTimer = 3600; //1 minute
		...
		//plasma beam
			int type6 = mod.TileType("PlasmaBeam");
			if(CheckTile(TileX, TileY, (ushort)type6))
			{
				havePlasma = true;
				Item.NewItem(new Vector2(TileX*16,TileY*16),8,8,mod.ItemType("WaveBeam"),1,false,-1,false,false);
				upgradePercent++;
				for(int i = 1; i < 3; i++) 
				{
					if(Main.tile[TileX, TileY+i].type == type6) WorldGen.KillTile(TileX, TileY+i, false, false, true);
				}
				if(ffTimer <= 0)
				{
					ffTimer = 3600; //1 minute
		...
		//speed booster
			int type7 = mod.TileType("SpeedBooster");
			if(CheckTile(TileX, TileY, (ushort)type7))
			{
				haveSpeed = true;
				Item.NewItem(new Vector2(TileX*16,TileY*16),8,8,mod.ItemType("SpeedBooster"),1,false,-1,false,false);
				upgradePercent++;
				for(int i = 1; i < 3; i++) 
				{
					if(Main.tile[TileX, TileY+i].type == type7) WorldGen.KillTile(TileX, TileY+i, false, false, true);
				}
				if(ffTimer <= 0)
				{
					ffTimer = 3600; //1 minute
		...
		//hi-jump
			int type8 = mod.TileType("HiJump");
			if(CheckTile(TileX, TileY, (ushort)type3))
			{
				haveJump = true;
				Item.NewItem(new Vector2(TileX*16,TileY*16),8,8,mod.ItemType("HiJump"),1,false,-1,false,false);
				upgradePercent++;
				for(int i = 1; i < 3; i++) 
				{
					if(Main.tile[TileX, TileY+i].type == type8) WorldGen.KillTile(TileX, TileY+i, false, false, true);
				}
				if(ffTimer <= 0)
				{
					ffTimer = 3600; //1 minute
		...
		//screw attack
			int type9 = mod.TileType("ScrewAttack");
			if(CheckTile(TileX, TileY, (ushort)type9))
			{
				haveWave = true;
				Item.NewItem(new Vector2(TileX*16,TileY*16),8,8,mod.ItemType("ScrewAttack"),1,false,-1,false,false);
				upgradePercent++;
				for(int i = 1; i < 3; i++) 
				{
					if(Main.tile[TileX, TileY+i].type == type9) WorldGen.KillTile(TileX, TileY+i, false, false, true);
				}
				if(ffTimer <= 0)
				{
					ffTimer = 3600; //1 minute
		...
		//space jump
			int type10 = mod.TileType("SpaceJump");
			if(CheckTile(TileX, TileY, (ushort)type10))
			{
				haveWave = true;
				Item.NewItem(new Vector2(TileX*16,TileY*16),8,8,mod.ItemType("SpaceJump"),1,false,-1,false,false);
				upgradePercent++;
				for(int i = 1; i < 3; i++) 
				{
					if(Main.tile[TileX, TileY+i].type == type10) WorldGen.KillTile(TileX, TileY+i, false, false, true);
				}
				if(ffTimer <= 0)
				{
					ffTimer = 3600; //1 minute
		...
		//xray scope
			int type11 = mod.TileType("XRayScope");
			if(CheckTile(TileX, TileY, (ushort)type11))
			{
				haveScope = true;
				Item.NewItem(new Vector2(TileX*16,TileY*16),8,8,mod.ItemType("XRayScope"),1,false,-1,false,false);
				upgradePercent++;
				for(int i = 1; i < 3; i++) 
				{
					if(Main.tile[TileX, TileY+i].type == type11) WorldGen.KillTile(TileX, TileY+i, false, false, true);
				}
				if(ffTimer <= 0)
				{
					ffTimer = 3600; //1 minute
		...
		//missile tank
			int type12 = mod.TileType("MissileTank");
			if(CheckTile(TileX, TileY, (ushort)type12))
			{
				missileUpg += 1;
				Item.NewItem(new Vector2(TileX*16,TileY*16),8,8,mod.ItemType("Missile"),5,false,0,false,false);
				upgradePercent++;
				for(int i = 1; i < 3; i++) 
				{
					if(Main.tile[TileX, TileY+i].type == type12) WorldGen.KillTile(TileX, TileY+i, false, false, true);
				}
				if(ffTimer <= 0)
				{
					ffTimer = 3600; //1 minute
		...
		//super missile tank
			int type13 = mod.TileType("SMissileTank");
			if(CheckTile(TileX, TileY, (ushort)type13))
			{
				smissileUpg += 1;
				Item.NewItem(new Vector2(TileX*16,TileY*16),8,8,mod.ItemType("SuperMissile"),5,false,0,false,false);
				upgradePercent++;
				for(int i = 1; i < 3; i++) 
				{
					if(Main.tile[TileX, TileY+i].type == type13) WorldGen.KillTile(TileX, TileY+i, false, false, true);
				}
				if(ffTimer <= 0)
				{
					ffTimer = 3600; //1 minute
		...
		//energy tank
			int type14 = mod.TileType("EnergyTank");
			if(CheckTile(TileX, TileY, (ushort)type14))
			{
				...
				for(int i = 1; i < 3; i++) 
				{
					if(Main.tile[TileX, TileY+i].type == type14) WorldGen.KillTile(TileX, TileY+i, false, false, true);
				}
				if(ffTimer <= 0)
				{
					ffTimer = 3600; //1 minute
		...
		//	spikes detection
			...
			if(CheckTile((int)tilev.X, (int)tilev.Y, (ushort)spikes))
			{
				PlayerDeathReason PDR = new PlayerDeathReason();
		...
		public bool CheckTile(int i, int j, ushort type)
		{
			bool Top = (type == Main.tile[i, j+1].type);
			bool Middle = (type == Main.tile[i, j+2].type);
			bool Bottom = (type == Main.tile[i, j+3].type);
		...
		//	POWER SUIT
		//	textures
			Helmet = mod.GetTexture("Gores/Armor/PowerSuitHead");
			Body = mod.GetTexture("Gores/Armor/PowerSuitBody");
			Legs = mod.GetTexture("Gores/Armor/PowerSuitLegs");
		//	draw
			if(!player.invis && PowerSuit && !VariaSuit && !GravitySuit)
			{
		...
		//	VARIA SUIT
		//	textures
			Helmet = mod.GetTexture("Gores/Armor/VariaSuitHead");
			Body = mod.GetTexture("Gores/Armor/VariaSuitBody");
			Legs = mod.GetTexture("Gores/Armor/VariaSuitLegs");
		//	draw
			if(!player.invis && VariaSuit && !GravitySuit)
			{
				if(player.direction == 1)
				{
				//draw head
		...
		//	GRAVITY SUIT
		//	textures
			Helmet = mod.GetTexture("Gores/Armor/GravitySuitHead");
			Body = mod.GetTexture("Gores/Armor/GravitySuitBody");
			Legs = mod.GetTexture("Gores/Armor/GravitySuitLegs");
		//	draw
			if(!player.invis && ((VariaSuit && GravitySuit) || (!VariaSuit && GravitySuit)))
			{
				if(player.direction == 1)
				{
				//draw head
			...
			//	Main.spriteBatch.Draw(powerArmor, 
			//	new Vector2((float)((int)(player.position.X - Main.screenPosition.X - (float)(player.bodyFrame.Width / 2) + (float)(player.width / 2))), (float)((int)(player.position.Y - Main.screenPosition.Y + (float)player.height - (float)player.bodyFrame.Height + 4f))) + player.bodyPosition + new Vector2((float)(player.bodyFrame.Width / 2), (float)(player.bodyFrame.Height / 2)), 
			//	new Rectangle?(player.bodyFrame), default(Color), rotation, origin, 1f, effects, 0f);
	...
	{	
		...
		{
			if(!PowerSuit && !VariaSuit && !GravitySuit)
			{
				PlayerLayer.HairBack.visible = true;
				PlayerLayer.Skin.visible = true;
				PlayerLayer.Body.visible = true;
				PlayerLayer.Face.visible = true;
				PlayerLayer.Hair.visible = true;
				PlayerLayer.Head.visible = true;
				PlayerLayer.Arms.visible = true;
				PlayerLayer.Legs.visible = true;
				PlayerLayer.ShoeAcc.visible = true;
			}
			else
			{
...
// archived code storage
			/*	// armor accessory (manually changing player armor inventory)
			#region armor
			if(item.type == mod.ItemType("PowerArmor") || item.type == mod.ItemType("VariaArmor") || item.type == mod.ItemType("GravityArmor"))
			{
				PowerSuit = true;
			...
			if(item.type == mod.ItemType("PowerArmor") && variaUpg == 1)
			{
				if(Main.GetKeyState((int)Microsoft.Xna.Framework.Input.Keys.R) < 0 && switchTime <= 0)
				{
					...
					player.armor[0].SetDefaults(mod.ItemType("VariaSuitHelmet"), false);
					player.armor[1].SetDefaults(mod.ItemType("VariaSuitBreastplate"), false);
					player.armor[2].SetDefaults(mod.ItemType("VariaSuitGreaves"), false);
				}
			}
			if(item.type == mod.ItemType("PowerArmor") && variaUpg != 1 && gravityUpg == 1)
			{
				if(Main.GetKeyState((int)Microsoft.Xna.Framework.Input.Keys.R) < 0 && switchTime <= 0)
				{
					...
					player.armor[0].SetDefaults(mod.ItemType("GravitySuitHelmet"), false);
					player.armor[1].SetDefaults(mod.ItemType("GravitySuitBreastplate"), false);
					player.armor[2].SetDefaults(mod.ItemType("GravitySuitGreaves"), false);
				}
			}
			if(item.type == mod.ItemType("PowerArmor") && variaUpg != 1 && gravityUpg != 1)
			{
				if(Main.GetKeyState((int)Microsoft.Xna.Framework.Input.Keys.R) < 0 && switchTime <= 0)
				{
					switchTime = 45;
					Main.PlaySound(mod.GetLegacySoundSlot(SoundType.Custom, "Sounds/Custom/Click"), player.position);
					player.armor[0].SetDefaults(mod.ItemType("PowerSuitHelmet"), false);
					player.armor[1].SetDefaults(mod.ItemType("PowerSuitBreastplate"), false);
					player.armor[2].SetDefaults(mod.ItemType("PowerSuitGreaves"), false);				
				}
			}
