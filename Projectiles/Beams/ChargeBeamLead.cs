...
{
	public class ChargeBeamLead : ModProjectile
	{
		public override void SetStaticDefaults()
		{
			DisplayName.SetDefault("Charge Beam Lead");
		}
		...
		#region sounds and dust
			...
			if(CHARGE > 99)
			{
				...
				int dust = Dust.NewDust(P.position+P.velocity, P.width, P.height, 64, 0, 0, 100, color, 2.0f);
		...
		public override void Kill(int timeleft)
		{
			...
			float PlayerCentreX = O.position.X + O.width/2;
			float PlayerCentreY = O.position.Y + O.height/2;
			int manaCost = 25 - (int)((float)O.inventory[O.selectedItem].mana*O.manaCost);
			if(CHARGE > 99 && O.statMana >= manaCost && O.height == 42)
			{
				Main.PlaySound(mod.GetLegacySoundSlot(SoundType.Item, "Sounds/Item/PowerBeamChargeShot"), projectile.position);
				Pindex = Projectile.NewProjectile(PlayerCentreX,PlayerCentreY,P.velocity.X,P.velocity.Y,mod.ProjectileType("ChargeBeam"),(int)((float)40*O.rangedDamage),5,P.owner);
				O.manaRegenDelay = (int)O.maxRegenDelay;
			}
			else if(O.height == 42)
			{
				Main.PlaySound(mod.GetLegacySoundSlot(SoundType.Item, "Sounds/Item/PowerBeam"), projectile.position);
				Pindex = Projectile.NewProjectile(PlayerCentreX,PlayerCentreY,P.velocity.X,P.velocity.Y,mod.ProjectileType("PowerBeam"),(int)((float)10*O.rangedDamage),5,P.owner);
			}
			if(O.height == 14 && CHARGE > 99 && O.statMana > 9)
			{
				...
				if(O.statMana >= 10) O.statMana -= 10;
				O.manaRegenDelay = (int)O.maxRegenDelay;
			}
